    ...
    return (
      <div className="text-center py-8">
        <p className="text-sm text-slate-500">
          Henüz zaman çizelgesi verisi yok
        </p>
      </div>
    );
  }
        ...
        <div className="relative pt-6">
          {sortedEvents.map((event) => {
            ...
            const positionPercent =
              durationMs > 0 ? (elapsed / durationMs) * 100 : 0;
            ...
            else if (event.eventType.includes("start"))
              bgColor = "bg-yellow-500";
            else if (event.eventType.includes("think"))
              bgColor = "bg-purple-500";
            else if (event.eventType.includes("search"))
            ...
            return (
              ...
              >
                <div
                  className={`${bgColor} w-2 h-2 rounded-full animate-pulse`}
                />
              </div>
            );
          })}
