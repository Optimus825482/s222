...
export function XpTaskbar({
  ...
  return (
    <div className="xp-taskbar h-[40px] flex items-stretch shrink-0 relative z-[9999]">
              ...
              {Array.from(groups.entries()).map(([groupName, groupApps]) => (
                <div key={groupName}>
                  ...
                  {groupApps.map((app) => (
                    ...
                    >
                      <span className="w-8 h-8 flex items-center justify-center text-blue-600 group-hover:text-white shrink-0">
                        {app.icon}
                      </span>
      ...
      {startCtx && (
        <div
          className="fixed z-[250] bg-white rounded shadow-[2px_2px_8px_rgba(0,0,0,0.3)] border border-gray-300 py-1 min-w-[200px] max-w-[90vw] text-[12px] text-gray-800"
          style={{
