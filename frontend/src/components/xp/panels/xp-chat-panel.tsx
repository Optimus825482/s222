...
export default function ChatDesktopPanel() {
  ...
  const { status, sendMessage, sendOrchestratorChat, stop } = useAgentSocket({
    enabled: !!user,
    ...
    onStreamEvent: (ev) => {
      ...
      switch (ev.event_type) {
        ...
        case "toolcall_delta":
          appendStreamToolCallArgs(
            (ev.extra.tool_call_id as string) || "",
            ev.delta,
          );
          break;
        case "toolcall_end":
          updateStreamToolCall((ev.extra.tool_call_id as string) || "", {
            status: "complete",
          });
          break;
        case "done":
